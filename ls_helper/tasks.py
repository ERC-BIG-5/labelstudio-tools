...
def strict_update_project_task_data(
    task_id: int, new_data: dict, existing_data: Optional[dict] = None
):
    client = ls_client()
    if existing_data:
    ...
    if resp.status_code != 200:
        raise Exception(
            f"Error, task with id: {task_id} does not exist or could not be updated"
        )
